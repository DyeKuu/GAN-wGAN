...
def viz_gaussian_train(uniform, gaussian_data):
    ...
    # uniform_grid = np.linspace(0, 1, n_data)

    ax1.plot(uniform_sorted, data_sorted)
    # ax1.plot(uniform_grid, stats.norm.ppf(uniform_grid))
...
def viz_gaussian_gan_unidim(uniform, gaussian_data, generator, discriminator,  list_loss_G, list_loss_D, epoch):
    ...
    uniform_idx_sorted = uniform.argsort(axis=0)
    uniform_sorted= uniform[uniform_idx_sorted].ravel()
    ...
    # PDF
    sns.distplot(gaussian_data, ax=ax2, kde=True, label="réelle ($\mu$={:.2f}, $\sigma$={:.2f})".format(
        gaussian_data.mean(), gaussian_data.std()))
    sns.distplot(generated_data, ax=ax2, kde=True, label="GAN ($\mu$={:.2f}, $\sigma$={:.2f})".format(
...
def viz_gaussian_gan_multidim(uniform, gaussian_data, generator, discriminator, list_loss_G, list_loss_D, epoch):
    ...
    # PDF
    sns.distplot(gaussian_data, ax=ax1, kde=True, label="réelle ($\mu$={:.2f}, $\sigma$={:.2f})".format(
        gaussian_data.mean(), gaussian_data.std()))
    sns.distplot(generated_data, ax=ax1, kde=True, label="GAN ($\mu$={:.2f}, $\sigma$={:.2f})".format(
        generated_data.numpy().mean(), generated_data.numpy().std()))
